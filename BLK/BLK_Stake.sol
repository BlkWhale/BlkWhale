...
contract Deposit is Ownable {
    ...
    function setToken(address BLK_, address USDB_, address FLY_, address pair_,address U_) public onlyOwner {
        BLK = IERC20(BLK_);
        USDB = IERC20(USDB_);
        FLY = IERC20(FLY_);
        BLKPair = pair_;
    ...
    function getBLKPrice(address pair) public view returns (uint price_){
        uint token = BLK.balanceOf(pair);
        uint uu = IERC20(u).balanceOf(pair);
        price_ = token * 1e18 / uu;
        // reserve0 A, reserve 1 U, it is U price of A
        return price_;
    }
    ...
    function claimInterest(uint slot_) public {
        ...
        if (userSlot[msg.sender][slot_].mode == 1) {
            ...
            userInfo[checkUserInvitor(msg.sender)].inviteRewardBLK += temp * 3 / 100;
            ...
            userSlot[msg.sender][slot_].depositTime = block.timestamp;
            userInfo[checkUserInvitor(msg.sender)].inviteRewardUSDB += temp * 3 / 100;
            userSlot[msg.sender][slot_].claimed += temp;
        }
    ...
    function setUserInvitorReward(address addr_, uint BLK_, uint USDB_) public isAdmin {
        require(checkUserInvitor(addr_) != address(0), 'wrong address');
